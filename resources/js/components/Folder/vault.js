...
class Folder extends React.Component {
  ...
  onSubmit = event => {
    console.log(this.state);
    const { name, category, username, password, url, notes } = this.state;
  ...
  onUpdate = event => {
    console.log(this.state);
    const { id, name, category, username, password, url, notes } = this.state;
    event.preventDefault();
    if(name  === "" || name === null) {
      this.setState({nameError: "Please enter name", errors: '', success: ''});
    }
  ...
  getAllItems = () => {
    axios.get(Config.get_single_folder + folderId, { headers: {"authorization" : localStorage.getItem('token')} })
      .then(({ data }) => {
        const state = { ...this.state }
...
export default withParams(Folder);